...
describe FindNumber do
  ...
  describe '#game_over?' do
    context 'when guess and random_number are equal' do
      ...
      subject(:check_game) { described_class.new(0, 9, number_end, 4) }
      ...
      it 'is game over' do
        ...
        expect(check_game).to be_game_over
      end
    end
    ...
    context 'when guess and random_number are not equal' do
      let(:number_end) { double( 'random_number') }
      subject(:check_game) { described_class.new(0, 9, number_end, 7) }
      it 'is not game over' do
        allow(number_end).to receive(:value).and_return(8)
        expect(check_game).not_to be_game_over
      end
    end
  end
  ...
  describe '#update_range' do
    ...
    context 'when the guess is less than the answer' do
      ...
      it 'updates min to 5' do
        # expect(low_guess_game.update_range).to change { low_guess_game.min }.to eq(5)
    ...
    context 'when the guess is more than the answer' do
      ...
      it 'updates max to 8' do
        # expect(high_guess_game.update_range).to change { high_guess_game.max }.to eq(8)
