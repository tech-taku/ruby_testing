...
describe Drink do
  ...
  describe '#full?' do
    context 'when drink has 16 ounces or more' do
      # Create an explicit subject, using 'described_class' and your choice of
      # beverage type.
      subject(:my_chibuku) { described_class.new('masese', 20) }
      # remove the 'x' before running this test
      it 'is full' do
        expect(my_chibuku).to be_full
      end
    end
    ...
    context 'when drink has less than 16 ounces' do
      ...
      subject(:my_super) {described_class.new('yorghut yabab', 5)}
      # remove the 'x' before running this test
      it 'is not full' do
        expect(my_super).not_to be_full
      end
    end
  end
end
